# Smart Plug Energy-Tracking App 🔌 


## Overview
This project demonstrates an IoT-enabled smart plug designed to monitor and track energy consumption.  
The solution provides real-time usage data and visualizations to help users optimize energy consumption.

## Features
- Real-time energy usage monitoring  
- Historical consumption tracking  
- User-friendly data visualization interface  
- Market strategy analysis for deployment and adoption  

## My Contribution (Rohit Saravanan)
- **Scrum Master**: Directed a 9-member team, organizing sprints, managing deliverables, and ensuring on-time project delivery.  
- **System Documentation**: Produced detailed design reports and diagrams showcasing the solution architecture.  
- **Marketing Strategy & Plan**: Developed a comprehensive marketing and adoption plan, outlining value proposition, target market, and go-to-market approach.  
- **Presentation & Delivery**: Coordinated the final submission and received *exceptional faculty recognition* for leadership and execution.  

## Disclaimer
This project was co-developed as part of a group coursework by 9 students.  
Due to academic integrity policies, the source code is not included.  
This repository serves as a **portfolio showcase** of my personal contributions and leadership.  
